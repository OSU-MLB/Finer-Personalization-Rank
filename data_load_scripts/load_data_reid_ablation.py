...
"""
Script that loads wildlife10k data into a standard format for miewID. Still needs to go through format_data.py/ manual viewpoint check prior for some datasets, such as CowDataset.
Copy of load_data_reid.py with modifications for ablation
"""
...
def main():
    ...
    for specie in species:#takes largest #per_species subjects per specie
        ...
        name_counts = name_counts = specie_df['name'].value_counts()
    ...
    df = df[df['file_path'].isin(train_df['file_path']) == False] #remove training data from df so it isnt used in gallery
    ...
    df = df.groupby('name', group_keys=False).apply(lambda x: x.sample(n=min(len(x), 15), random_state=args.seed))
    ...
    gallery_df = pd.concat([df,gallery_df])
